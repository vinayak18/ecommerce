...
//	}
	...
	@Test
	@Order(4)
	public void productionSearchStatus() throws Exception {
	...
	@Test
	@Order(5)
	public void productionSearchWithoutKeyword() throws Exception {
		mvc.perform(get("/api/public/product/search")).andExpect(status().is(400));
	}
	
	@Test
	@Order(6)
	public void productionSearchWithProductName() throws Exception {
	...
	@Test
	@Order(7)
	public void productionSearchWithCategoryName() throws Exception {
	...
	public void consumerApiWithSellerJWT() throws Exception {
		mvc.perform(get("/api/auth/consumere").header("JWT", loginHelper(c_u, p).getContentAsString()))
		.andExpect(status().is(403));
	}
	...
	public void sellerAddNewProduct() throws Exception {
		createdURI = mvc.perform(post("/api/auth/seller/product").header("JWT", loginHelper(c_u, p).getContentAsString())
	...
	public void sellerCheckAddedNewProduct() throws Exception {
		mvc.perform(get(new URL(createdURI).getPath()).header("JWT", loginHelper(c_u, p).getContentAsString()))
	...
	public void sellerUpdateProduct() throws Exception {
		...
		mvc.perform(put("/api/auth/seller/product").header("JWT", loginHelper(c_u, p).getContentAsString())
				.contentType(MediaType.APPLICATION_JSON)
	...
	public void consumerUpdateProductInCartWithZeroQuantity() throws Exception {
		...
		mvc.perform(get("/api/auth/consumer/cart").header("JWT", loginHelper(c_u, p).getContentAsString()))
			...
			.andExpect(jsonPath("$.cartProducts[1].quantity", is(2)))
			.andExpect(jsonPath("$.cartProducts[1].product.productName", containsStringIgnoringCase("Crocin pain relief tablet")))
			.andExpect(jsonPath("$.cartProducts[1].product.category.categoryName", is("Medicines")));
	}
	...
	public void sellerDeleteProduct() throws Exception {
		String[] arr = createdURI.split("/");
		mvc.perform(delete("/api/auth/seller/product/" + Integer.valueOf(arr[arr.length - 1])).header("JWT", loginHelper(c_u, p).getContentAsString()))
			.andExpect(status().is(200));

		mvc.perform(get("/api/auth/seller/product/" + Integer.valueOf(arr[arr.length - 1])).header("JWT", loginHelper(c_u, p).getContentAsString()))
			.andExpect(status().is(404));
	}
}
